...
describe('Compositor Performance', () => {
  ...
  bench('medium complexity (10 objects)', () => {
    const c = new Compositor();
    for (let i = 0; i < 10; i++) {
      c.addObject(new AsciiObject({ id: `obj${i}`,
        content: [['#']],
        position: { x: i, y: i }
      }));
    }
  ...
  bench('multiple layers with stacking effects', () => {
    ...
    for (let layer = 0; layer < 3; layer++) {
      c.addObject(new AsciiObject({ id: `obj${layer}`,
        content: [['#']],
  ...
  bench('large content object (50x50 grid)', () => {
    ...
    c.addObject(new AsciiObject({ id: 'large',
      content,
      position: { x: 0, y: 0 }
    }));
  ...
  bench('horizontal flip', () => {
    ...
    c.getObject('obj').flipHorizontalToggle();
  });
  ...
  bench('vertical flip', () => {
    ...
    c.getObject('obj').flipVerticalToggle();
  });

  // AutoDetectEdges
  bench('autoDetectEdges on complex shape', () => {
    const c = new Compositor();
    c.addObject(new AsciiObject({ id: 'shape',
      content: [
        ['  ', '  ', '  ', '  ', '  '],
        ['  ', '#', '#', '#', '  '],
        ['  ', '#', ' ', '#', '  '],
  ...
  warmCompositor.addObject(new AsciiObject({ id: 'obj',
    content: [['#']],
    position: { x: 0, y: 0 }
  }));
